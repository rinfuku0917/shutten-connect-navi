    ...
    <div style={{minHeight:'100vh',background:'#FFF9E6'}}>
      ...
      <div style={{background:'#F5A623',display:'flex'}}>
        {[{label:'ホーム',href:'/'},{label:'出店したい',href:'/vendor'},{label:'お店を呼びたい',href:'/space'},{label:'出店者を探す',href:'/sellers'},{label:'出店場所を探す',href:'/places'},{label:'車両を売りたい',href:'/sell'}].map((item,i,arr)=>(
          <Link key={item.label} href={item.href} style={{flex:1,color:'#fff',fontWeight:'900',fontSize:'13px',padding:'12px 0',textAlign:'center',textDecoration:'none',borderRight:i<arr.length-1?'1px solid rgba(255,255,255,0.3)':'none',background:item.href==='/vendor'?'rgba(0,0,0,0.15)':'transparent'}}>
            {item.label}
          </Link>
        ))}
      </div>
