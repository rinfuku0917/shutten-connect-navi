-- ⚠️ このファイルは流さないこと（2026-09-19 追記）
--
--   このファイルは supabase/migrations/20260919_public_sellers_no_name.sql で
--   置き換わった。下の定義には本名の列（name）が入っており、
--   revoke も書いていないため、単体で流し直すと
--     ・出店者1,386人ぶんの本名が anon キーで誰でも読める状態
--     ・anon の INSERT / UPDATE（profiles 本体まで届く書き込み）
--   の両方が戻る。
--   create or replace view は冪等に見えるが、この2つが戻るという意味で
--   「流し直しても害は無い」ファイルではない。
--   public_sellers を作り直すときは 20260919 のほうを流すこと。
--
